...
MIN_FRAMES = 15   # minimum detections before a student's data is trusted & saved
...
class ScanPage(tk.Frame):
    ...
    def _update_summary(self):
        ...
        for sid, emotions in self._session_emotions.items():
            ...
            n    = len(emotions)
            if n < MIN_FRAMES:
                lines.append(f"  {name}: ⏳ {n}/{MIN_FRAMES} frames  (building…)")
            else:
                ws        = wellness_score(emotions)
                lbl, _col = wellness_label(ws)
                lines.append(f"  {name}: {ws}%  [{lbl}]  ({n} frames)")
    ...
    def _stop_and_save(self):
        ...
        for sid, emotions in self._session_emotions.items():
            if sid not in db:
                continue
            if len(emotions) < MIN_FRAMES:
                skipped_names.append(
                    f"{db[sid]['name']} (only {len(emotions)} frames — likely misidentified)"
                )
                continue
            ws          = wellness_score(emotions)
        ...
        skip_note = ""
        if skipped_names:
            skip_note = (f"\n\nSkipped ({len(skipped_names)} below {MIN_FRAMES}-frame threshold):\n"
                         + "\n".join(skipped_names))
        self._status.set(
